...
#from psychopy import logging # remove psychopy logging
import struct
import sys
import time
import numpy
...
class PR650(object):
    ...
    def __init__(self, port, verbose=None):
        ...
        if self.OK:
            ...
            try:
                ...
                self.isOpen = 1
        if self.OK:
            print("Successfully opened %s" % self.portString)
    ...
    def sendMessage(self, message, timeout=0.5, DEBUG=False):
        ...
        print(message)  # send complete message
        if message in ('d5', 'd5\n'):
    ...
    def measure(self, timeOut=30.0):
        ...
        if reply == self.codes['OK']:
            ...
            print("Didn't collect any data (extend timeout?)")
